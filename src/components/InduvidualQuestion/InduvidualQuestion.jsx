...
function InduvidualQuestion() {
  ...
  const individualQuestions = async()=>{
    try {
        const res = await axios.get(`https:localhost:44355/api/Questions/${id}`)
...
const comments = async()=>{
  try {
      const res = await axios.get(`https://localhost:44355/api/Comments`)
...
const handleSubmit = (event)=>{
  ...
  const generateNewComment= async(newComment,FKquestionid)=>{
      ...
      try {
          const res = await axios.post('https://localhost:44355/api/Comments',{
